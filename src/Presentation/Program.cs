...
builder.Services.AddScoped<ITornApiService>(sp => sp.GetRequiredService<TornApiService>());
builder.Services.AddHttpClient<TornStatsApiService>(client => client.BaseAddress = new Uri(TornStatsApiAddress));
builder.Services.AddScoped<ITornStatsApiService>(sp => sp.GetRequiredService<TornStatsApiService>());
